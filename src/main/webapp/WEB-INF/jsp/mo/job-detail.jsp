...
<div class="card">
    ...
    <div class="job-card-head">
        <div>
            <h2><c:out value="${empty job.title ? 'Untitled draft job' : job.title}"/></h2>
            <p class="muted">
    ...
    <c:if test="${param.published eq '1'}">
        <div class="success">Draft published successfully. TA users can now see this job in the available jobs list.</div>
    </c:if>
    ...
    <c:if test="${job.draft}">
        <div class="info">This job is still a draft. It remains private to the MO side until you publish it from the edit page.</div>
    </c:if>
    ...
    <div class="actions-row">
        ...
        <a class="button-secondary" href="${pageContext.request.contextPath}/mo/jobs/edit?jobId=${job.id}">
            <c:out value="${job.draft ? 'Continue Draft' : 'Edit Job'}"/>
        </a>
